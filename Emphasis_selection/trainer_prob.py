...
def visualize_attention(wts,words,filename):
    """
    A function create heat maps for prediction, ground truth and attention (if any)
    :param wts:
    :param words:
    :param filename:
    :return:
...
def to_tensor_labels(encodings,  return_mask=False):
    ...
    for i, sample in enumerate(encodings):
        ...
        # if config.bio_learning:
        #     pad = [[1, -1, -1]] * padding_len
        # else:
        pad = [[1,0]] * padding_len
        sample.extend(pad)
...
class Trainer(object):
        ...
        for epoch in range(self.epochs):
            ...
            with open("output_train.txt", "w") as f:
                for batch_i in range(total_batch_train):
                    ...
                    #         actual_words = words
                    #     else:
                    #         actual_words = [[self.encoder.index2word[elem] for elem in elems] for elems in words]
                    #     for w in actual_words:
                    #         f.write(str(w)+ " \n")
                    #     f.write("\n mask: \n")
                    #     f.write(str(mask_numpy)+ "\n")
                    #     f.write("\n prediction: \n")
                    #     f.write(str(numpy_predictions_max))
                    #     f.write("\n\n true label max-ed: \n")
                    #     f.write(str(numpy_label_max))
                    #     f.write("\ncheck prediction: \n")
                    #     f.write("\n overlaped: \n")
                    #     f.write(str(overlaped))
                    #     f.write("\n scores_flat: \n")
                    #     f.write(str(torch.exp(scores_flat)))
                    #     f.write("\n target_flat_softmaxed: \n")
                    #     f.write(str(target_flat_softmaxed))
                    #
            ...
            # ^^^^^^^^^^^^^^^^^^^^^^^^^^
            self.batch_size = self.batch_size_org
            print("[LOG] dev total size: ****** ", len(self.corpus.dev.labels))
            # ^^^^^^^^^^^^^^^^^^^^^^^^^^
            dev_right_preds = 0
            dev_total_preds = 0
            total_dev_loss = 0
    ...
    def predict(self, model, theLoss, theCorpus):
        ...
        if len(theCorpus.words) % self.batch_size > 0:
            # print("len(theCorpus.words) % self.batch_size >0")
            total_batch_test += 1
            # ^^^^^^^^^^^^^^^^^^^^^^^^^^
